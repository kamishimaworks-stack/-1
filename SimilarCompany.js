...
以下のJSON形式で回答してください:
{
   ...
   * @param {number} [limit=10] - 処理件数の上限
   */
  function batchAnalyze(limit) {
    const maxLimit = limit || 10;
    const customers = SheetHelper.getAllCustomers();

    // 類似企業カラムが空の顧客のみ対象
    const targets = customers.filter(c =>
      c.companyName && !c.challenges  // challengesが空 = まだ分析未実施
    ).slice(0, maxLimit);
...
function batchSimilarCompanyAnalysis() {
  const result = SimilarCompanyFinder.batchAnalyze(10);
