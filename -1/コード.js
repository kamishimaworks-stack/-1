...
function api_runBatchSimilar() {
  return SimilarCompanyFinder.batchAnalyze();  // 全件処理
}


// ==========================================
// Microsoft 365 連携 API ブリッジ
// ==========================================

/** Microsoft 認証 URL を取得 */
function api_getMicrosoftAuthUrl() {
  return MicrosoftAuth.getAuthorizationUrl();
}

/** Microsoft 認証状態を確認 */
function api_isMicrosoftAuthorized() {
  return MicrosoftAuth.isAuthorized();
}

/** Microsoft 認証を解除 */
function api_microsoftLogout() {
  MicrosoftAuth.logout();
  return { success: true };
}

/** 現在のメールプロバイダー設定を返す */
function api_getEmailProvider() {
  return SFA_CONFIG.MICROSOFT.EMAIL_PROVIDER || 'gmail';
}

/** 手動 Outlook 同期を実行 */
function api_syncOutlookContacts() {
  return OutlookService.syncAllContactDates();
}
...
/**
 * 日次 Outlook 同期 (トリガー用 - 7:00 実行)
 * 休眠チェック (9:00) の前にメール履歴を同期する
 */
function dailyOutlookSync() {
  try {
    if (!MicrosoftAuth.isAuthorized()) {
      console.log('[dailyOutlookSync] Microsoft 未認証のためスキップ');
      return;
    }
    const result = OutlookService.syncAllContactDates();
    console.log(`Outlook同期結果: ${JSON.stringify(result)}`);
  } catch (e) {
    console.error('Outlook同期でエラーが発生しました: ' + e.message);
  }
}

/**
 * Outlook 同期トリガーの設定 (毎日 7:00)
 */
function setupOutlookSyncTrigger() {
  // 既存の同名トリガーを削除
  const triggers = ScriptApp.getProjectTriggers();
  triggers.forEach(t => {
    if (t.getHandlerFunction() === 'dailyOutlookSync') {
      ScriptApp.deleteTrigger(t);
    }
  });

  // 毎日 AM7:00 に実行
  ScriptApp.newTrigger('dailyOutlookSync')
    .timeBased()
    .everyDays(1)
    .atHour(7)
    .create();

  console.log('Outlook同期トリガーを設定しました (毎日 7:00 実行)');
}
...
/**
 * 既存顧客のSNS検索URLを最新の検索ロジックで一括更新する。
 * GASエディタから手動で実行してください。
 */
function batchUpdateSnsUrls() {
  const sheet = SheetHelper.getMainSheet();
  ...
  data.forEach((row, i) => {
    ...
    // 既存の企業サイトURLを渡してGemini呼び出しを回避
    const existingSite = row[COL.COMPANY_SITE] || row[COL.WEBSITE] || '';
    // 既存の推定業種があればメイン事業キーワードとして渡す（API節約）
    const existingIndustry = row[COL.INDUSTRY] || '';
    const snsInfo = SnsResearch.generateSearchUrls(fullName, companyName, existingSite, existingIndustry);

    const rowNum = i + 2; // 1-based, ヘッダー分+1
    sheet.getRange(rowNum, COL.X_URL + 1).setValue(snsInfo.xUrl || '');
    sheet.getRange(rowNum, COL.FACEBOOK_URL + 1).setValue(snsInfo.facebookUrl || '');
    sheet.getRange(rowNum, COL.INSTAGRAM_URL + 1).setValue(snsInfo.instagramUrl || '');
    sheet.getRange(rowNum, COL.YOUTUBE_URL + 1).setValue(snsInfo.youtubeUrl || '');
    sheet.getRange(rowNum, COL.TIKTOK_URL + 1).setValue(snsInfo.tiktokUrl || '');
    updatedCount++;
  });

  console.log('SNS検索URL更新完了: ' + updatedCount + '件');
}
