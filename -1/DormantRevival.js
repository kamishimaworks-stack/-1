  ...
  function processAll() {
    ...
    targets.forEach(customer => {
      try {
        ...
        // 4. 下書き保存 (Emailがある場合) — プロバイダー設定に基づき振り分け
        if (customer.email) {
          _saveDraft(customer.email, draft.subject, draft.body);
        }
    ...
    // Teams 通知
    if (SFA_CONFIG.ENV.TEAMS_WEBHOOK_URL) {
      try {
        TeamsNotification.sendBatchResult({
          taskName: '休眠顧客メール生成',
          processed: processedCount,
          total: dormants.length,
          errors: targets.length - processedCount,
        });
      } catch (e) {
        console.warn('[DormantRevival] Teams通知失敗: ' + e.message);
      }
    }
...
何卒よろしくお願いいたします。`;
   ...
   * @param {string} body    - 本文
   */
  function _saveOutlookDraft(to, subject, body) {
    try {
      OutlookService.createDraft(to, subject, body);
      console.log(`[DormantRevival] Outlook下書き作成: → ${to}`);
    } catch (e) {
      console.warn('[DormantRevival] Outlook下書き作成スキップ: ' + e.message);
    }
  }

  /**
   * EMAIL_PROVIDER 設定に基づき下書きを振り分け
   * @param {string} to      - 宛先
   * @param {string} subject - 件名
   * @param {string} body    - 本文
   */
  function _saveDraft(to, subject, body) {
    const provider = SFA_CONFIG.MICROSOFT.EMAIL_PROVIDER || 'gmail';

    if (provider === 'gmail' || provider === 'both') {
      _saveGmailDraft(to, subject, body);
    }
    if (provider === 'outlook' || provider === 'both') {
      _saveOutlookDraft(to, subject, body);
    }
  }
